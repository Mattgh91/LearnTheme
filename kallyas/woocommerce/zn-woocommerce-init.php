	...
	if ( ! function_exists( 'zn_woocommerce_get_product_thumbnail' ) ) {
			...
			$placeholder_height = 0 )
		{
			...
			if ( has_post_thumbnail($post->ID) ) {
				...
				$image = vt_resize( $thumbid, '', $width, $height, true );
                $imageUrl = $image['url'];
                ...
                if(!empty($imageUrl)){
					...
					// TODO: Decide if we're going to use Attachment Alt & Title
...
function zn_woocommerce_before_main_content(){
	...
	echo '<section id="content" class="site-content shop_page">';
...
if(!function_exists('zn_woocommerce_after_main_content')){
	function zn_woocommerce_after_main_content(){
		...
		if( is_archive() || is_shop() || (( is_single() && $woo_prod_page_layout != 'style3')) ){
			?>
					</div>
					<!-- sidebar -->
					<?php get_sidebar(); ?>
